...
class Sources {
	...
	addServer(networkId) {
		const chosenNetwork = client.networks[networkId];

		select('#message-source-list').insertAdjacentHTML('beforeend',
			client.Templates['public/views/sources.hbs']({
				networkId,
				network: chosenNetwork,
				networkName: chosenNetwork.networkName || chosenNetwork.serverGiven || 'Server',
			})
		);
	...
	addToList(networkId, source) {
		...
		select(`${networkSelector} .channel-source-container`).insertAdjacentHTML('beforeend',
			client.Templates['public/views/source.hbs']({
				networkId,
				source,
				type,
	...
	changeFocus(networkId, source, type) {
		...
		client.networks.focusedServer = networkId;
		...
		switch (type) {
			...
			case 'pm':
				...
				// Disable the user from being able to edit it (there is no point to)
